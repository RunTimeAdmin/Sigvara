...
Sigvara shipped its own on-chain registries: `SigvaraIdentity`
(a `did:sigvara` DID registry) and `SigvaraReputation` (a computed
6-factor score), plus `SigvaraStaking` (bonds + slashing).
...
Maintaining a parallel `did:sigvara` identity registry alongside the
...
1. **ERC-8004 Identity Registry is the canonical agent registry going forward.**
   The `did:sigvara` identity method is **deprecated**. Agents are identified
   by their ERC-8004 agent id (CAIP-10 style: `eip155:{chainId}:{registry}` +
   agentId).

2. **`SigvaraReputation` is retained** and reframed as the **computed-score
...
3. **`SigvaraStaking` is retained** as the staked-accountability
   (bonding + slashing) layer. ERC-8004 has no equivalent; this is the
   differentiator.

4. **`SigvaraIdentity` is deprecated as a registry.** Its only non-redundant
...
- The deployed `SigvaraIdentity` on Arc testnet `5042002` remains for
...
- **The full code cutover** (oracle keying on the 8004 agent id, SDK, and app
  ...
  `/link` bridge (Sigvara agent ↔ 8004 agent id, ownership-verified) carries
  the interop.

## What this does not change

`SigvaraReputation` and `SigvaraStaking` stay. The redundancy was only
ever the identity registry; the computation and accountability layers are the
reason Sigvara exists on top of the standard.
