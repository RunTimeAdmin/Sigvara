...
$('btnDispute').addEventListener('click', function () { attempt(function () { sim.disputeSlash(); out('outSlash', line('disputed', 'proposal handed to the committee', 'warn') + line('bond', 'stays frozen; withdrawals remain blocked') + line('agent', 'stays Suspended') + line('next', 'the committee upholds or rejects, or anyone may expire it after 14 days')); }, 'outSlash'); });
$('btnUphold').addEventListener('click', function () { attempt(function () { sim.resolveDispute(true); out('outSlash', line('upheld', 'the slash proceeds', 'bad') + line('burned (50%)', D.fmtToken(sim.burned) + ' SVR total to 0xdead') + line('status', 'Slashed, terminal', 'bad') + line('note', 'victim and reporter shares are credited for them to claim')); }, 'outSlash'); });
$('btnRejectDispute').addEventListener('click', function () { attempt(function () { sim.resolveDispute(false); out('outSlash', line('rejected', 'proposal dropped, agent reinstated to Active', 'ok') + line('bond', 'released; withdrawals work again')); }, 'outSlash'); });
$('btnExpire').addEventListener('click', function () { attempt(function () { sim.expireDispute(); out('outSlash', line('expired', 'the committee never resolved it', 'ok') + line('result', 'agent reinstated, bond released') + line('note', 'permissionless, so a freeze cannot be indefinite')); }, 'outSlash'); });
