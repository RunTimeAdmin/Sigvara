...
This protocol redesign transitions Sigvara from a centralized SaaS architecture into a permissionless, cryptoeconomically secured infrastructure. The design philosophy centers on **verifiability, interoperability, and economic accountability**. By anchoring W3C Decentralized Identifiers (DIDs) on-chain, enforcing Ed25519 PKI challenge-response authentication, and implementing a staked 6-factor reputation algorithm, Sigvara establishes a trustless environment for agent-to-agent (A2A) and human-to-agent interactions.
...
- **Economic Security:** Replaces subscription billing with a staked-bond model (token design undecided: purpose-built or standard ERC-20), introducing slashing mechanics to penalize malicious agent behavior.
...
Sigvara operates across three layers: the On-Chain State Layer, the Decentralized Identity Layer, and the Off-Chain Verification Layer.
...
- **Decentralized Identity Layer:** A network of DID resolvers that map `did:sigvara` identifiers to their on-chain state and retrieve public keys for signature verification.
...
Sigvara implements a custom DID method, `did:sigvara`, fully compliant with W3C standards, optimized for Ed25519 keypairs.

### DID Method Specification
- **Format:** `did:sigvara:<chain_id>:<agent_address>`
...
2. **Challenge:** Agent B generates a payload: `SIGVARA-VERIFY:{AgentA_DID}:{Nonce}:{Timestamp}` and sends it to Agent A.
...
The Sigvara reputation system computes a deterministic score (0-100) using a 6-factor model. This score is stored in the Reputation Registry and determines the agent's trust tier.
...
> **Phase note:** External Trust (4) and Trust Propagation (6) are stubbed to 0 until the Phase 2 oracle network is live; the on-chain caps and the other four factors are active today. This matches `docs/reputation-model.md`, `SigvaraReputation.sol`, and `oracle/scoring.js`.
...
The protocol relies on a staked bond token (design undecided) to align incentives, prevent Sybil attacks, and provide economic recourse for malicious behavior.

### Staking and Bonding Mechanisms
- **Identity Bond:** To register an agent in the Identity Registry, the operator must stake a minimum amount of stake tokens. This stake is locked for the duration of the agent's active status.
...
- **Validator Yield:** Independent nodes that operate the reputation oracle network must stake stake tokens. They earn yield from query fees paid by applications resolving DIDs and fetching reputation scores.
- **Attestation Rewards:** Counterparties who submit verifiable cryptographic proofs of successful agent interactions receive a micro-reward in stake tokens, incentivizing accurate data reporting.
...
2. The Operator calls `depositStake()` on the Staking contract with the required stake tokens.
3. The Operator calls `registerAgent()` on the Identity Registry, linking the agent's public key to their Ethereum address.
4. The protocol emits `AgentRegistered`, and the agent's `did:sigvara` identifier is now resolvable globally.
...
Integrating an AI agent with Sigvara requires utilizing the SDK to manage identity, sign challenges, and query peer reputation.

### SDK Overview
The `@sigvara/protocol-sdk` provides abstraction over the EVM contracts and Ed25519 cryptography.

### Example: Agent Authentication
```typescript
import { SigvaraAgent, ChallengeResponse } from '@sigvara/protocol-sdk';

// Initialize the agent with its private key
const agent = new SigvaraAgent({
  privateKey: process.env.AGENT_ED25519_PRIVATE_KEY,
  rpcUrl: 'https://rpc.sigvara.xyz'
});

// 1. Receive challenge from a peer
const challengePayload = "SIGVARA-VERIFY:did:sigvara:1:0xabcd:nonce-123:1710000000";
...
const verifier = new SigvaraVerifier({ rpcUrl: 'https://rpc.sigvara.xyz' });
...
- **Sybil Attacks:** Mitigated by the stake tokens staking requirement. Creating 10,000 fake agents requires significant capital lockup, making it economically irrational.
...
- **Phase 1: Core Protocol (Q3 2026)**
  - Deploy Identity, Reputation, and Staking registries to Arc testnet.
  - Release v1.0 of `@sigvara/protocol-sdk`.
...
- **Phase 3: Mainnet Launch (Q1 2027)**
  ...
  - Mainnet deployment; bond-asset decision (custom token vs. established asset).
- **Phase 4: Cross-Chain Expansion (Q2 2027)**
  - Deploy state-mirroring contracts to Solana and Base via LayerZero.

### Open Questions & Design Decisions
- `[OPEN QUESTION]` **Slashing Adjudication:** Who has the final say on slashing? A decentralized court (like Kleros), an optimistic oracle (like UMA), or a dedicated Sigvara validator set? *Tradeoff:* Speed of resolution vs. decentralization of power.
...
- `[TBD]` **ERC-8004 Alignment:** While Sigvara shares the Identity/Reputation/Validation registry structure with ERC-8004, it deviates by enforcing economic staking. We must decide whether to strictly implement the ERC-8004 interface for interoperability, or define a new standard (e.g., ERC-8004-Stake) to accommodate the cryptoeconomic requirements.
