...
> **Implemented, and broken since 17 September 2026.** The enrichment, the outcome-to-oracle path and the ERC-8004 publisher all exist in CounterAudit. They were written before this protocol was renamed from Countersig, and they still parse `did:countersig:` and derive the DID hash from that prefix. A current `did:sigvara:` DID is rejected outright, and even bypassing that, the hash would not match what the contract holds, so the agent would read as unregistered. Renaming the protocol changed what the hash commits to; this code was never updated to follow. It is also undocumented in CounterAudit's `openapi.yaml`, which is why the feature is easy to miss.

How it works, once the scheme is corrected: an ingest call carrying `agent_did` makes CounterAudit query the Sigvara contracts at seal time and embed the agent's identity and reputation score inside the AES-GCM seal, covered by an RFC 3161 timestamp — proving what the agent's reputation was at the moment of each action rather than today. An ingest also carrying an `outcome` (`success` / `failure`) is reported to the oracle, feeding audited work outcomes back into Success Rate and Fee Activity. Reading and writing the same reputation is what closes the loop.
