...
>
> **The reverse path is not live.** CounterAudit posts outcomes to `/attest` without a `payment.txHash`, which the oracle requires, so they are rejected. That is a design gap rather than a naming one: CounterAudit does not hold the settlement transaction for the work it audits.
