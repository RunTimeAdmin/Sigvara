...
[CounterAudit](https://counteraudit.io) is the first planned integration. It shares an owner with Sigvara, as part of the [CounterAegis](https://counteraegis.com) suite, so it is not an arm's-length third party — see [docs/ecosystem.md](docs/ecosystem.md).

> **Not built yet.** `agent_did` appears nowhere in CounterAudit's published API, and no enrichment or outcome-reporting code exists on its side. Until 19 September 2026 this section described the integration in the present tense, which was wrong.

The design, once built: an ingest call carrying `agent_did` would make CounterAudit query the Sigvara contracts at seal time and embed the agent's identity and reputation score inside the AES-GCM seal, covered by an RFC 3161 timestamp — proving what the agent's reputation was at the moment of each action rather than today. An ingest also carrying an `outcome` (`success` / `failure`) would be reported to the oracle, feeding audited work outcomes back into Success Rate and Fee Activity. Reading and writing the same reputation is what would close the loop.
