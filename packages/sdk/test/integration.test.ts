/**
 * Integration test harness for @sigvara/protocol-sdk
 *
 * Requires a live EVM deployment (Arc testnet 5042002 by default). Auto-skips when env vars are absent
 * so CI is never blocked.
 *
 * To run after deploy (see docs/arc.md):
 *   SIGVARA_RPC_URL=https://rpc.testnet.arc.io
 *   SIGVARA_CHAIN_ID=5042002
 *   SIGVARA_IDENTITY_ADDRESS=0x...
 *   SIGVARA_REPUTATION_ADDRESS=0x...
 *   SIGVARA_STAKING_ADDRESS=0x...
 *   SIGVARA_OPERATOR_PRIVATE_KEY=0x...
 *
...
const RPC_URL         = process.env.SIGVARA_RPC_URL;
const IDENTITY_ADDR   = process.env.SIGVARA_IDENTITY_ADDRESS;
const REPUTATION_ADDR = process.env.SIGVARA_REPUTATION_ADDRESS;
const STAKING_ADDR    = process.env.SIGVARA_STAKING_ADDRESS;
const OPERATOR_KEY    = process.env.SIGVARA_OPERATOR_PRIVATE_KEY;

const ENABLED = !!RPC_URL && !!IDENTITY_ADDR && !!REPUTATION_ADDR && !!STAKING_ADDR && !!OPERATOR_KEY;
const maybeDescribe = ENABLED ? describe : describe.skip;

// Default Arc testnet; override with SIGVARA_CHAIN_ID (Arc mainnet is 5042).
const CHAIN_ID = Number(process.env.SIGVARA_CHAIN_ID ?? '5042002');

let verifier: SigvaraVerifier;
...
maybeDescribe('Integration: A2A challenge-response', () => {
  it('verifySignature returns true for valid signature', async () => {
    const agent = new SigvaraAgent({ privateKey: agentPrivateKey, agentAddress, chainId: CHAIN_ID });
