...
// Challenge format: SIGVARA-VERIFY:{proverDid}:{nonce}:{timestamp}
// The DID may contain colons (did:sigvara:1:0x...), so we parse from the right.

export function generateChallenge(proverDid: string, ttlSeconds = DEFAULT_TTL): Challenge {
  const nonce = randomHex(16);
  const timestamp = Math.floor(Date.now() / 1000);
  const payload = `SIGVARA-VERIFY:${proverDid}:${nonce}:${timestamp}`;
