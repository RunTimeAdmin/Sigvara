...
export function parseDid(did: string): ParsedDid {
  const match = did.match(/^did:sigvara:(\d+):(0x[0-9a-fA-F]{40})$/);
  if (!match) throw new Error(`Invalid did:sigvara format: ${did}`);
...
export function formatDid(agentAddress: string, chainId: number | bigint): string {
  return `did:sigvara:${chainId}:${agentAddress.toLowerCase()}`;
}
...
export function computeDidHash(agentAddress: string, chainId: number | bigint): string {
  return ethers.keccak256(
    ethers.solidityPacked(
      ...
      ['did:sigvara:', BigInt(chainId), ':', agentAddress]
    )
  );
}
