...
Sigvara ships as an **oracle and registry service**, not a token
launch. There is no $SVR token sale, TGE, or listing planned. Anyone selling
a "$SVR" token is running a scam.
...
- **The testnet keeps using the faucet SVRToken** for mechanics testing. It
  has no value and never will.
- **The TGE contract set stays on the shelf.** SVR (fixed-supply), vesting,
