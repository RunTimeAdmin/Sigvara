...
/**
 * @title SigvaraPublicSale
 * @notice Fixed-price sale of the 15% public allocation (tokenomics v0.3 §2).
 *         Buyers pay a stablecoin at a fixed price for $SVR.
 *
 ...
 *             maxPerWallet per address. The contract must hold >= hardCap $SVR.
 *           - finalize() (after end, or early once hardCap is hit):
 *               * softCap met  -> SUCCESS: stablecoin proceeds go to treasury,
 *                 unsold $SVR is burned to 0xdead (§2 "unsold ... permanently
 ...
 */
contract SigvaraPublicSale is ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public immutable svr;
    IERC20 public immutable paymentToken;
    uint256 public immutable price;        // payment-token units per 1e18 (1 whole) SVR
    uint64 public immutable startTime;
    uint64 public immutable endTime;
    uint256 public immutable hardCap;      // max SVR to sell (wei)
    uint256 public immutable softCap;      // min SVR sold for success (wei)
    uint256 public immutable maxPerWallet; // max SVR per address (wei)
    ...
    mapping(address => uint256) public purchased; // SVR owed to a buyer
    ...
    event Finalized(bool succeeded, uint256 tokensSold, uint256 svrMoved);
    ...
    constructor(
        ...
        if (svr_ == address(0) || paymentToken_ == address(0) || treasury_ == address(0)) {
            revert ZeroAddress();
        }
    ...
    /// @notice Stablecoin cost of `tokenAmount` SVR (wei). Rounded up.
    ...
    /// @notice True once the contract holds enough $SVR to honor the full hard cap.
    function isFunded() public view returns (bool) {
        return svr.balanceOf(address(this)) >= hardCap;
    }

    /**
     * @notice Buy `tokenAmount` SVR (wei) at the fixed price. Caller must approve
    ...
    function finalize() external nonReentrant {
        ...
        uint256 funded = svr.balanceOf(address(this));

        if (succeeded) {
            // Sold $SVR stays for claim(); the remainder is unsold -> burned (§2).
            ...
            if (unsold > 0) svr.safeTransfer(BURN_ADDRESS, unsold);
            emit Finalized(true, tokensSold, unsold);
        } else {
            // No sale happened: buyers refund their stablecoin (kept in the contract),
            // and the whole $SVR allocation returns to treasury rather than being burned.
            if (funded > 0) svr.safeTransfer(treasury, funded);
            emit Finalized(false, tokensSold, funded);
        }
    }

    /// @notice Claim purchased $SVR after a successful sale.
