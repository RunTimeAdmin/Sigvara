...
/**
 * @title DeployMainnet
 * @notice Full TGE deployment for the fixed-supply mainnet $SVR token.
 *
 * Deploys, in order:
 ...
 *   4. SVR            — mints 1B once and distributes to the five buckets:
...
contract DeployMainnet is Script {
    ...
    function run() external {
        ...
        require(svr.totalSupply() == svr.TOTAL_SUPPLY(), "supply != 1B");
        require(svr.balanceOf(address(treasuryVesting)) == svr.TREASURY_ALLOCATION(),  "treasury bal");
        require(svr.balanceOf(address(teamVesting)) == svr.TEAM_ALLOCATION(),          "team bal");
        require(svr.balanceOf(address(timelock)) == svr.ECOSYSTEM_ALLOCATION(),        "ecosystem bal");
        require(svr.balanceOf(publicSale) == svr.PUBLIC_SALE_ALLOCATION(),             "public bal");
        require(svr.balanceOf(liquidity) == svr.LIQUIDITY_ALLOCATION(),                "liquidity bal");
        ...
        console2.log("=== Sigvara Mainnet TGE Deployment ===");
