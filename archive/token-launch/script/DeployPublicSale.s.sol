 ...
 * @notice Deploys the immutable SigvaraPublicSale. All terms are fixed at
 *         construction. After deploy, fund the sale with `SALE_HARD_CAP` $SVR
 ...
 *   SVR_ADDRESS       — $SVR token
 *   PAYMENT_TOKEN      — stablecoin buyers pay with (e.g. USDC)
 *   TREASURY_ADDRESS   — receives proceeds on success
 *   SALE_PRICE         — payment-token units per 1e18 SVR (e.g. USDC 6dp @ $0.01 = 10000)
 *   SALE_START         — unix start timestamp
 *   SALE_END           — unix end timestamp
 *   SALE_HARD_CAP      — max SVR to sell (wei)
 *   SALE_SOFT_CAP      — min SVR sold for success (wei)
 *   SALE_MAX_WALLET    — max SVR per address (wei)
 */
contract DeployPublicSale is Script {
    function run() external {
        ...
        SigvaraPublicSale sale = new SigvaraPublicSale(
            svr, payment, price, startTime, endTime, hardCap, softCap, maxWallet, treasury
        );
        vm.stopBroadcast();

        require(address(sale.svr()) == svr, "svr not set");
        require(sale.hardCap() == hardCap, "hardCap not set");
        require(sale.treasury() == treasury, "treasury not set");

        console2.log("=== Sigvara Public Sale Deployment ===");
        ...
        console2.log("Fund the sale with SALE_HARD_CAP SVR before it opens.");
    }
}
