...
contract SVRTest is Test {
    ...
    function setUp() public {
        token = new SVR(treasury, team, ecosystem, publicSale, liquidity);
    }
    ...
    function test_metadata() public view {
        assertEq(token.name(), "Sigvara");
        assertEq(token.symbol(), "SVR");
    ...
    function test_constructor_reverts_zeroTreasury() public {
        vm.expectRevert(SVR.ZeroAddress.selector);
        new SVR(address(0), team, ecosystem, publicSale, liquidity);
    }

    function test_constructor_reverts_zeroLiquidity() public {
        vm.expectRevert(SVR.ZeroAddress.selector);
        new SVR(treasury, team, ecosystem, publicSale, address(0));
    }
